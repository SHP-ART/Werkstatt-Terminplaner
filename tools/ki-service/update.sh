...
if [ ! -d "$INSTALL_DIR" ]; then
    echo -e "${RED}Fehler: KI-Service nicht gefunden in $INSTALL_DIR${NC}"
...
if [ -d ".git" ]; then
    ...
    cp -r $INSTALL_DIR/tools/ki-service $BACKUP_DIR
...
cd tools/ki-service
$INSTALL_DIR/venv/bin/pip install -r requirements.txt --upgrade --quiet
